...
const ApresentarMidias = () => {
  ...
  const fetchImages = async () => {
    try {
      ...
      setImages(response.data); // Define as imagens no estado
    } catch (err) {
  ...
  return (
    <div>
     ...
     <a className="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
      href="/upload"> fazer upload de midia</a>
      <div>
      <h1>Mídias</h1>
      {error && <p style={{ color: 'red' }}>{error}</p>}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
        {images.map((url, index) => (
          <img key={index} src={url} alt={`Imagem ${index}`} style={{ width: '150px', height: '150px', objectFit: 'cover' }} />
        ))}
      </div>
    </div>
    </div>
